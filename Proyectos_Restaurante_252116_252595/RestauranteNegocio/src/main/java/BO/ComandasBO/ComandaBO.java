...
1. **Constructor**: `ComandaBO(ComandasDAO comandasDAO, ProductosDAO productosDAO, ClientesDAO clientesDAO, MesasDAO mesasDAO)`
2. **Parámetros**: 
   - `comandasDAO`: Un objeto de tipo `ComandasDAO` que se utilizará para realizar operaciones relacionadas con las comandas en la base de datos.
   - `productosDAO`: Un objeto de tipo `ProductosDAO` que se utilizará para realizar operaciones relacionadas con los productos en la base de datos.
   - `clientesDAO`: Un objeto de tipo `ClientesDAO` que se utilizará para realizar operaciones relacionadas con los clientes en la base de datos.
   - `mesasDAO`: Un objeto de tipo `MesasDAO` que se utilizará para realizar operaciones relacionadas con las mesas en la base de datos.
...
1. **Método**: `generarFolioComandaBO()`
2. **Tipo de retorno**: `String`
3. **Funcionalidad**:
   - Llama al método `generarFolioComanda` del `comandasDAO` para obtener un nuevo folio que se utilizará para identificar la comanda de manera única.
...
1. **Método**: `modificarComandaBO(NuevaComandaDTO comandaDTO, List<NuevoDetalleComandaDTO> detallesDTO)`
2. **Parámetros**: 
   - `comandaDTO`: Un objeto de tipo `NuevaComandaDTO` que contiene la información de la comanda a modificar, incluyendo su folio.
   - `detallesDTO`: Una lista de objetos de tipo `NuevoDetalleComandaDTO` que representan los detalles de los productos incluidos en la comanda.
3. **Tipo de retorno**: `void`
4. **Excepciones**: 
   - Lanza `IllegalArgumentException` si `comandaDTO` es `null`, o si el folio de la comanda es `null` o está vacío.
   - Lanza `IllegalArgumentException` si `detallesDTO` es `null` o está vacío, indicando que debe haber al menos un detalle de comanda.
   - Lanza `RuntimeException` si no hay suficiente stock para modificar la comanda.

5. **Funcionalidad**:
   - Se verifica que el objeto `comandaDTO` no sea `null` y que su folio no esté vacío. Si alguna de estas condiciones no se cumple, se lanza una excepción.
   - Se verifica que la lista de detalles no esté vacía. Si lo está, se lanza una excepción.
   - Se llama al método `verificarStockNecesarioProductos` del `comandasDAO` para comprobar si hay suficiente stock para los productos en los detalles de la comanda. Si no hay suficiente stock, se lanza una excepción.
   - Se itera sobre cada detalle de la comanda para calcular el total de cada detalle utilizando el método `calcularTotalDetalleComanda` del `comandasDAO`, y se establece el importe total en cada detalle.
   - Se calcula el total de la comanda utilizando el método `calcularTotalComanda` del `comandasDAO` y se establece en el objeto `comandaDTO`.
...
1. **Método**: `eliminarComandaBO(NuevaComandaDTO comandaDTO)`
2. **Parámetro**: 
   - `comandaDTO`: Un objeto de tipo `NuevaComandaDTO` que contiene la información de la comanda a eliminar, incluyendo su folio.
3. **Tipo de retorno**: `void`
4. **Excepciones**: 
   - Lanza `IllegalArgumentException` si `comandaDTO` es `null`, o si el folio de la comanda es `null` o está vacío, indicando que se requiere un folio válido para realizar la eliminación.
...
1. **Método**: `cancelarComandaBO(String folio)`
2. **Parámetro**: 
   - `folio`: Un `String` que representa el folio de la comanda que se desea cancelar.
3. **Tipo de retorno**: `void`
4. **Excepciones**: 
   - Lanza `IllegalArgumentException` si el `folio` es `null` o está vacío, indicando que se requiere un folio válido para realizar la cancelación.

5. **Funcionalidad**:
   - Se verifica que el `folio` no sea `null` y que no esté vacío. Si alguna de estas condiciones no se cumple, se lanza una excepción.
...
2. **Parámetro**: 
   - `folio`: Un `String` que representa el folio de la comanda que se desea marcar como entregada.
3. **Tipo de retorno**: `void`
4. **Excepciones**: 
   - Lanza `IllegalArgumentException` si el `folio` es `null` o está vacío, indicando que se requiere un folio válido para realizar la entrega.
...
1. **Método**: `verificarStockNecesarioProductosBO(List<NuevoDetalleComandaDTO> detallesDTO)`
2. **Parámetro**: 
   - `detallesDTO`: Una lista de objetos de tipo `NuevoDetalleComandaDTO` que representan los detalles de los productos cuya disponibilidad de stock se desea verificar.
3. **Tipo de retorno**: `boolean`
4. **Excepciones**: 
   - Lanza `IllegalArgumentException` si `detallesDTO` es `null` o está vacía, indicando que la lista de detalles de comanda no puede estar vacía.

5. **Funcionalidad**:
   - Se verifica que la lista de detalles no sea `null` y que no esté vacía. Si alguna de estas condiciones no se cumple, se lanza una excepción.
...
1. **Método**: `restarStockIngredientesPorProductosComandaBO(List<NuevoDetalleComandaDTO> detallesDTO)`
2. **Parámetro**: 
   - `detallesDTO`: Una lista de objetos de tipo `NuevoDetalleComandaDTO` que representan los detalles de los productos para los cuales se desea restar el stock de ingredientes.
3. **Tipo de retorno**: `void`
4. **Excepciones**: 
   - Lanza `IllegalArgumentException` si `detallesDTO` es `null` o está vacía, indicando que la lista de detalles de comanda no puede estar vacía.
...
1. **Método**: `modificarNotaBO(NuevoDetalleComandaDTO detalleDTO, String nuevaNota)`
2. **Parámetros**: 
   - `detalleDTO`: Un objeto de tipo `NuevoDetalleComandaDTO` que representa el detalle de la comanda que se desea modificar. Debe contener el folio de la comanda y el nombre del producto.
   - `nuevaNota`: Un `String` que representa la nueva nota que se desea asignar al detalle de la comanda.
3. **Tipo de retorno**: `void`
4. **Excepciones**: 
   - Lanza `IllegalArgumentException` si `detalleDTO` es `null`, o si el folio de la comanda o el nombre del producto son `null`, indicando que el detalle de comanda debe contener esta información.
   - Lanza `IllegalArgumentException` si `nuevaNota` es `null`, indicando que la nueva nota no puede ser nula.

5. **Funcionalidad**:
   - Se verifica que el objeto `detalleDTO` no sea `null` y que contenga tanto el folio de la comanda como el nombre del producto. Si alguna de estas condiciones no se cumple, se lanza una excepción.
   - Se verifica que `nuevaNota` no sea `null`. Si lo es, se lanza una excepción.
...
1. **Método**: `mostrarComandasTodasBO()`
2. **Tipo de retorno**: `List<Comanda>`
3. **Funcionalidad**:
   - Se crea una instancia de `ComandasDAO`.
...
1. **Método**: `mostrarComandasAbiertasBO()`
2. **Tipo de retorno**: `List<Comanda>`
3. **Funcionalidad**:
   - Se crea una instancia de `ComandasDAO`.
...
2. **Parámetros**: 
   - `fechaInicio`: Un objeto de tipo `Calendar` que representa la fecha de inicio del rango para filtrar las comandas.
   - `fechaFin`: Un objeto de tipo `Calendar` que representa la fecha de fin del rango para filtrar las comandas.
3. **Tipo de retorno**: `List<Comanda>`
4. **Excepciones**: 
   - Lanza `IllegalArgumentException` si `fechaInicio` o `fechaFin` son `null`, indicando que ambas fechas son necesarias para realizar el filtrado.
   - Lanza `IllegalArgumentException` si `fechaInicio` es posterior a `fechaFin`, indicando que el rango de fechas es inválido.
   - Lanza `RuntimeException` si no se encuentran comandas en el rango de fechas especificado.

5. **Funcionalidad**:
   - Se verifica que las fechas de inicio y fin no sean `null`. Si alguna de estas condiciones no se cumple, se lanza una excepción.
   - Se verifica que `fechaInicio` no sea posterior a `fechaFin`. Si lo es, se lanza una excepción.
   - Se llama al método `filtrarPorFecha` del `comandasDAO`, pasando las fechas para obtener la lista de comandas que se encuentran dentro del rango especificado.
...
1. **Método**: `obtenerComandaPorFolioBO(String folio)`
2. **Parámetro**: 
   - `folio`: Un `String` que representa el folio de la comanda que se desea obtener.
3. **Tipo de retorno**: `Comanda`
4. **Excepciones**: 
   - Lanza `NegocioException` si ocurre un error en la lógica al buscar la comanda, específicamente si se produce una `PersistenciaException` durante la operación de búsqueda.

5. **Funcionalidad**:
   - Se llama al método `obtenerComandaPorFolio` del `comandasDAO`, pasando el `folio` para recuperar la comanda correspondiente.
...
1. **Método**: `obtenerListaDetallesComandaBO(Comanda comanda)`
2. **Parámetro**: 
   - `comanda`: Un objeto de tipo `Comanda` que representa la comanda de la cual se desea obtener la lista de detalles.
3. **Tipo de retorno**: `List<DetalleComanda>`
4. **Excepciones**: 
   - Lanza `NegocioException` si ocurre un error al recuperar los detalles de la comanda, específicamente si se produce una `PersistenciaException` durante la operación de recuperación.

5. **Funcionalidad**:
   - Se llama al método `obtenerListaDetallesComanda` del `comandasDAO`, pasando la `comanda` para recuperar la lista de detalles asociados a ella.
...
1. **Método**: `actualizarClienteFrecuente(Long idCliente, double totalGastado)`
2. **Parámetros**: 
   - `idCliente`: Un `Long` que representa el identificador único del cliente que se desea actualizar.
   - `totalGastado`: Un `double` que representa el total de dinero gastado por el cliente, que se utilizará para calcular los puntos acumulados.
3. **Tipo de retorno**: `void`
4. **Funcionalidad**:
   - Se calcula el número de puntos acumulados dividiendo el `totalGastado` entre 20. Cada 20 unidades monetarias gastadas equivalen a 1 punto.
   - Se establece el número de visitas a 1, asumiendo que esta actualización corresponde a una nueva visita del cliente.
   - Se crea una instancia de `ClientesDAO`.
