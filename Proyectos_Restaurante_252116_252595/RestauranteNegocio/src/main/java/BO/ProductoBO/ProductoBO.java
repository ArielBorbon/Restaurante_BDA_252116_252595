...
public class ProductoBO implements IProductoBO {
    ...
    /**
     * Este método se encarga de recuperar una lista de productos que están
     ...
     * excepciones, lo que implica que se asume que el `productoDAO` está
     * correctamente configurado para realizar la consulta.
     *
     * Este método es útil para obtener una lista de productos que están
     * disponibles para la venta, lo que es esencial en aplicaciones de gestión
    ...
    /**
     * Este método se encarga de filtrar la lista de productos disponibles por
     * su nombre.1.**Método**: `filtrarPorNombreProductoDisponiblesBO(String
     * nombreFiltro)` 2.
     *
     ...
     *
     * Este método es útil para buscar productos específicos en el inventario,
    ...
    /**
     * Este método se encarga de filtrar la lista de productos disponibles por
     * su tipo.1.**Método**: `filtrarPorTipoProductoDisponiblesBO(Tipo_Producto
     * tipoFiltro)` 2.
     *
    ...
    /**
     * Este método se encarga de filtrar la lista de productos disponibles por
     * su nombre y tipo.1.**Método**:
     * `filtrarPorNombreYTipoProductoDisponiblesBO(String nombreFiltro,
     * Tipo_Producto tipoFiltro)` 2.**Parámetros**: - `nombreFiltro`: Un
     ...
     * criterios especificados. - Este método no incluye validaciones ni manejo
     * de excepciones.
     *
     * Este método es útil para realizar búsquedas más específicas en el
    ...
    /**
     * Este método se encarga de recuperar una lista de todos los productos en
     ...
     *
     * Este método es útil para obtener una visión completa de todos los
    ...
    /**
     * Este método se encarga de registrar un nuevo producto en el
     * sistema.1.**Método**: `registrarProductoBO(NuevoProductoDTO
     * nuevoProducto)` 2.Parámetro**: - `nuevoProducto`: Un objeto de tipo
     * `NuevoProductoDTO` que contiene la información del nuevo producto a
     * registrar.
     *
     * 3. **Tipo de retorno**: `Producto` 4. **Excepciones**: - Lanza
     * `NegocioException` en los siguientes casos: - Si `nuevoProducto` es
     * `null`. - Si el nombre del producto es `null` o está vacío. - Si el
     * precio del producto es menor o igual a 0. - Si el tipo de producto es
     * `null`. - Si el estado del producto es `null`. - Si ya existe un producto
     * con el mismo nombre en el sistema.
     *
     * 5. **Funcionalidad**: - Se realizan varias validaciones sobre el objeto
     * `nuevoProducto` para asegurar que todos los campos requeridos estén
     * correctamente establecidos. - Se verifica si ya existe un producto con el
     * mismo nombre utilizando el método `buscarProductoPorNombre` del
     * `productoDAO`. Si se encuentra un producto existente, se lanza una
     * `NegocioException`. - Si todas las validaciones son exitosas, se llama al
     * método `registrarProducto` del `productoDAO` para registrar el nuevo
     * producto en la base de datos y se devuelve el objeto `Producto`
     * correspondiente.
     *
     * Este método es esencial para garantizar que solo se registren productos
    ...
    /**
     * Este método se encarga de deshabilitar un producto en el
     * sistema.1.**Método**: `deshabilitarProductoBO(NuevoProductoDTO
     * productoAEliminar)` 2.
     *
     * **Parámetro**: - `productoAEliminar`: Un objeto de tipo
     * `NuevoProductoDTO` que contiene la información del producto que se desea
     * deshabilitar. 3. **Excepciones**: - Lanza `NegocioException` en los
     * siguientes casos: - Si `productoAEliminar` es `null`. - Si el nombre del
     * producto es `null` o está vacío. - Si el precio del producto es menor o
     * igual a 0. - Si el tipo de producto es `null`. - Si ocurre un error al
     * intentar deshabilitar el producto en el DAO.
     *
     * 4. **Funcionalidad**: - Se realizan varias validaciones sobre el objeto
     * `productoAEliminar` para asegurar que todos los campos requeridos estén
     * correctamente establecidos. - Se llama al método `deshabilitarProducto`
     * del `productoDAO` para deshabilitar el producto en la base de datos. - Si
     * ocurre una excepción durante la operación de deshabilitación, se captura
     * y se lanza una nueva `NegocioException` con un mensaje que incluye la
     * causa del error.
     *
     ...
     * `NuevoProductoOcupaIngredienteDTO` que representan los ingredientes
     ...
     * `null`. - Si el nombre del producto es `null` o está vacío. - Si el
     * precio del producto es menor o igual a 0. - Si el tipo de producto es
     * `null`. - Si el estado del producto es `null`, se establece por defecto a
     * `Estado_Producto.HABILITADO`. - Si `listaProductoIngrediente` es `null` o
     * está vacía, indicando que el producto debe tener al menos un ingrediente
     * asociado. - Si ocurre un error al intentar registrar el producto con
     * ingredientes en el DAO.
     *
     * 5. **Funcionalidad**: - Se realizan varias validaciones sobre el objeto
     * `nuevoProducto` y la lista de ingredientes para asegurar que todos los
     * campos requeridos estén correctamente establecidos. - Se llama al método
     * `registrarProductoConIngredientes` del `productoDAO` para registrar el
     * nuevo producto junto con sus ingredientes en la base de datos. - Si
     * ocurre una excepción durante la operación de registro, se captura y se
     * lanza una nueva `NegocioException` con un mensaje que incluye la causa
     * del error.
     *
     * Este método es esencial para gestionar la creación de productos que
     ...
     * `NuevoProductoOcupaIngredienteDTO` que representan los nuevos
     * ingredientes asociados al producto.
     *
     * 3. **Tipo de retorno**: `Producto` 4. Excepciones**: - Lanza
     * `NegocioException` en los siguientes casos: - Si `nuevoProductoDTO` es
     * `null`. - Si el nombre del producto es `null` o está vacío. - Si
     * `nuevosIngredientesDTO` es `null`.
     *
     * 5. **Funcionalidad**: - Se realizan varias validaciones sobre el objeto
     * `nuevoProductoDTO` y la lista de nuevos ingredientes para asegurar que
     * todos los campos requeridos estén correctamente establecidos. - Se llama
     * al método `modificarProducto` del `productoDAO` para actualizar el
     * producto y sus ingredientes en la base de datos. - Se manejan excepciones
     * específicas, como `IllegalArgumentException`, y se lanza una
     * `NegocioException` con un mensaje que incluye la causa del error. También
     * se captura cualquier otra excepción inesperada y se lanza una
     * `NegocioException` genérica.
     *
     * Este método es esencial para permitir la actualización de productos en el
    ...
    /**
     * Este método se encarga de buscar un producto en el sistema por su
     * nombre.1.**Método**: `buscarProductoPorNombreBO(String nombre)` 2.
     *
     * Parámetro**: - `nombre`: Un `String` que representa el nombre del
     * producto que se desea buscar. 3. **Tipo de retorno**: `Producto` 4.
     * Funcionalidad**: - Llama al método `buscarProductoPorNombre` del
     * `productoDAO`, que se espera que devuelva el objeto `Producto`
     * correspondiente al nombre proporcionado. - Este método no incluye
     * validaciones ni manejo de excepciones, lo que implica que se asume que el
     * `productoDAO` está correctamente configurado para realizar la consulta.
     *
    ...
    /**
     * Este método se encarga de filtrar la lista de todos los productos por su
     * nombre.1.**Método**: `filtrarPorNombreProductoTodosBO(String
     * nombreFiltro)` 2.
     *
     * Parámetro**: - `nombreFiltro`: Un `String` que representa el nombre o
     * parte del nombre del producto que se desea filtrar. 3. **Tipo de
    ...
    /**
     * Este método se encarga de filtrar la lista de todos los productos por su
     * tipo.1.**Método**: `filtrarPorTipoProductoTodosBO(Tipo_Producto
     * tipoFiltro)` 2.
     *
     * **Parámetro**: - `tipoFiltro`: Un objeto de tipo `Tipo_Producto` que
     * representa el tipo de producto que se desea filtrar. 3. **Tipo de
    ...
    /**
     * Este método se encarga de filtrar la lista de todos los productos por su
     * nombre y tipo.1.**Método**: `filtrarPorNombreYTipoProductoTodosBO(String
     * nombreFiltro, Tipo_Producto tipoFiltro)` 2.**Parámetros**: -
     * `nombreFiltro`: Un `String` que representa el nombre o parte del nombre
     * del producto.
     *
     * -
    ...
    /**
     * Este método se encarga de habilitar un producto en el sistema.1.
     *
     * **Método**: `habilitarProductoBO(NuevoProductoDTO productoAHabilitar)`
     * 2. **Parámetro**: - `productoAHabilitar`: Un objeto de tipo
     * `NuevoProductoDTO` que contiene la información del producto que se desea
     * habilitar. 3. **Tipo de retorno**: `void` 4. **Funcionalidad**: - Llama
     * al método `habilitarProducto` del `productoDAO` para habilitar el
     * producto en la base de datos. - Este método no incluye validaciones ni
     * manejo de excepciones, lo que implica que se asume que el `productoDAO`
     * está correctamente configurado para realizar la operación.
     *
     * Este método es esencial para gestionar el estado de los productos en el
